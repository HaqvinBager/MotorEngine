...
PixelOutput main(VertexToPixel input)
{
    ...
    float3 albedo               = /*albedoTexture.Sample(defaultSampler, input.myUV.xy).rgb*/PixelShader_Albedo(input).myColor.rgb;
    float3 normal               = /*normalTexture.Sample(defaultSampler, input.myUV.xy).rgb*/ PixelShader_Normal(input).myColor.xyz;
    float3 metalness            = metalTexture.Sample(defaultSampler, input.myUV.xy).rgb;
    float3 perceptualRoughness  = roughTexture.Sample(defaultSampler, input.myUV.xy).rgb;
    float3 ambientocclusion     = aoTexture.Sample(defaultSampler, input.myUV.xy).rgb;
    float3 emissiveData         = emissiveTexture.Sample(defaultSampler, input.myUV.xy).rgb;
    
    float3 specularColor    = lerp((float3) 0.04f, albedo, metalness);
    float3 diffuseColor     = lerp((float3) 0.00f, albedo, 1 - metalness);
    
    float3 ambience         = EvaluateAmbience(environmentTexture, normal, input.myNormal.xyz, toEye, perceptualRoughness.r, metalness.r, albedo, ambientocclusion.r, diffuseColor, specularColor);
    float3 directionalLight = EvaluateDirectionalLight(diffuseColor, specularColor, normal, perceptualRoughness.r, directionalLightColor.xyz, toDirectionalLight.xyz, toEye.xyz);
    
    float3 pointLights = 0;
    //for (unsigned int i = 0; i < myNumberOfUsedPointLights; ++i)
    //{
    //    PointLight currentLight = myPointLights[i];
    ...
    //}
    //// To make PointLights more visible
    //ambience *= 0.1f;
    //directionalLight *= 0.01f;
    // ! To make PointLights more visible
    
    float3 emissive = albedo * emissiveData;
    float3 radiance = ambience + (directionalLight * directionalLightColor.a) + pointLights + emissive;
