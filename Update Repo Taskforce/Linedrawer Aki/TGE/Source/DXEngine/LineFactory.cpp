...
LineFactory* LineFactory::ourInstance = nullptr;

void LineFactory::Create()
{
	assert(ourInstance == nullptr && "LineFactory already created!");
	if( !ourInstance )
	{
		ourInstance = new LineFactory();
	}
}
...
void LineFactory::Init(DirectX11Framework* aFramework)
{
	assert(ourInstance != nullptr && "Instance has not been created.");
	if( !ourInstance )
	{
...
Line* LineFactory::CreateLine(const SM::Vector3& aFrom, const SM::Vector3& aTo, const SM::Vector4& aColor)
{
	HRESULT hResult;
...
Line* LineFactory::CreateGrid(const DirectX::SimpleMath::Vector4& aColor)
{
	HRESULT hResult;
