...
public:
	...
	const T& At(const int& aIndex) const;
	...
	void Pop();
	void RemoveCyclic(const int& aIndex);
	void Remove(const int& aIndex);

	void Reserve(const int& aCapacity);
	void Resize(const int& aCapacity);
	void Optimize();
...
// && in assert to remove error: too many arguments in function call
template<typename T>
inline T& Vector<T>::At(const int& aIndex) {
	assert(0 <= aIndex && "Index is less than 0!");
	assert(aIndex < mySize && "Trying to access with an index equal or larger than size!");
	return myData[aIndex];
}
template<typename T>
inline T& Vector<T>::operator[](const int& aIndex) {
	assert(0 <= aIndex && "Index is less than 0!");
	assert(aIndex < mySize && "Trying to access with an index equal or larger than size!");
	return myData[aIndex];
}
template<typename T>
inline const T& Vector<T>::At(const int& aIndex) const {
	assert(0 <= aIndex && aIndex < mySize && "Accessing element %d when vector has %d elements!" && aIndex && mySize);
	return myData[aIndex];
}
template<typename T>
inline const T& Vector<T>::operator[](const int& aIndex) const {
	assert(0 <= aIndex && aIndex < mySize && "Accessing element %d when vector has %d elements!" && aIndex && mySize);
	return myData[aIndex];
}
template<typename T>
...
template<typename T>
inline void Vector<T>::Insert(const T& aValue,const int& aIndex) {
...
template<typename T>
inline void Vector<T>::Reserve(const int& aCapacity) {
...
// min is a macro defined in windows.h, undef it here to allow use of std::min.
#undef min
template<typename T>
inline void Vector<T>::Resize(const int& aCapacity) {
	assert(aCapacity > 0 && "Can't have a vector with no capacity!");
	T* data = new T[aCapacity];
