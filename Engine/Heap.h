...
void CHeap<T, compare>::MoveUp(std::vector<T>& someData, int aChild)
{
	...
	while (aChild != 0 && myComparisonFunction(someData[aChild], someData[parent]))
	{ // If child value is greater than parent, swap
...
void CHeap<T, compare>::MoveDown(std::vector<T>& someData, int aFirst, int aLast)
{
	...
	while (largestChild <= aLast)
	{
		...
		if (myComparisonFunction(someData[largestChild], someData[aFirst]))
		{    // If node value is smaller than largest child value, swap      
